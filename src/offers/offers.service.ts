...
export class OffersService {
  ...
  async create(createOfferDto: CreateOfferDto, user: User): Promise<Offer> {
    ...
    if (wish.owner.id === user.id) {
      throw new HttpException('Вы не можете создавать предложения для своих подарков.', HttpStatus.FORBIDDEN);
    } else if (wish.price < wish.raised + amount) {
      throw new HttpException('Сумма собранных средств не может превышать стоимость подарка.', HttpStatus.BAD_REQUEST);
    }

    const newOffer = await this.offerRepository.create({ ...createOfferDto, user });
